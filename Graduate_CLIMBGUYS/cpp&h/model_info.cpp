...
)
{
	...
	if (pFile != NULL)
	{
		...
		while (strcmp(cHeadText, "END_SCRIPT") != 0)
		{
			...
			// キャラクターの当たり判定が来たら
			else if (strcmp(cHeadText, "CHARACTER_COLLISION") == 0)
			{
				// キャラクターの当たり判定情報の確保
				pmodelAll->pCharacterCollision = new CHARACTERCOLLISION;
				// エンドキャラクターの当たり判定が来るまでループ
				while (strcmp(cHeadText, "END_CHARACTER_COLLISION") != 0)
				{
					// 初期化
					cHeadText[0] = '\0';
					cRaedText[0] = '\0';
					...
					if (strcmp(cHeadText, "OFFSET") == 0)
					{
						sscanf(cRaedText, "%s %s %f %f %f", &cDie, &cDie,
							&pmodelAll->pCharacterCollision->Offset.x,
							&pmodelAll->pCharacterCollision->Offset.y,
							&pmodelAll->pCharacterCollision->Offset.z
						);
					}
					// 矩形情報が来たら
					...
					{
						// 矩形情報の確保
						pmodelAll->pCharacterCollision->RectInfo.reset(new RECTINFO);
						// エンドキャラクターの当たり判定が来るまでループ
						while (strcmp(cHeadText, "END_RECT") != 0)
						{
							...
							if (strcmp(cHeadText, "SIZE") == 0)
							{
								sscanf(cRaedText, "%s %s %f %f %f",
									&cDie, &cDie,
									&pmodelAll->pCharacterCollision->RectInfo->size.x,
									&pmodelAll->pCharacterCollision->RectInfo->size.y,
									&pmodelAll->pCharacterCollision->RectInfo->size.z
									);
							}
			...
			// 攻撃の当たり判定が来たら
			else if (strcmp(cHeadText, "ATTACK_COLLISION") == 0)
			{
				// 軌跡情報の確保
				pmodelAll->v_AttackCollision.push_back(ATTACKCOLLISION());
				// エンド軌跡セットが来るまでループ
				while (strcmp(cHeadText, "END_ATTACK_COLLISION") != 0)
				{
					// 初期化
					cHeadText[0] = '\0';
					cRaedText[0] = '\0';
					...
					sscanf(cRaedText, "%s", &cHeadText);
					// 線の数情報読み込み
					if (strcmp(cHeadText, "PARTS") == 0)
					{
						sscanf(cRaedText, "%s %s %d", &cDie, &cDie,
							&pmodelAll->v_AttackCollision.at(nCntAttackCollision).nParts);
					}
					// オフセット情報読み込み
					else if (strcmp(cHeadText, "OFFSET") == 0)
					{
						sscanf(cRaedText, "%s %s %f %f %f", &cDie, &cDie,
							&pmodelAll->v_AttackCollision.at(nCntAttackCollision).Offset.x,
							&pmodelAll->v_AttackCollision.at(nCntAttackCollision).Offset.y,
							&pmodelAll->v_AttackCollision.at(nCntAttackCollision).Offset.z
							);
					}
					// もし当たり判定の形がまだ決まっていなかったら入る
					else if (!pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_ColumnInfo ||
						!pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_RectInfo ||
						!pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_SphereInfo)
					{
						// 矩形情報が来たら
						if (strcmp(cHeadText, "RECT") == 0)
						{
							// 矩形情報の確保
							pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_RectInfo.reset(new RECTINFO);
							// エンド矩形の当たり判定が来るまでループ
							while (strcmp(cHeadText, "END_RECT") != 0)
							{
								// 初期化
								...
								if (strcmp(cHeadText, "SIZE") == 0)
								{
									sscanf(cRaedText, "%s %s %f %f %f",
										&cDie, &cDie,
										&pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_RectInfo->size.x,
										&pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_RectInfo->size.y,
										&pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_RectInfo->size.z
									);
								}
							}
						}
						...
						{
							// 円柱情報の確保
							pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_ColumnInfo.reset(new COLUMNINFO);
							// エンド円柱の当たり判定が来るまでループ
							while (strcmp(cHeadText, "END_COLUMN") != 0)
							{
								...
								if (strcmp(cHeadText, "RADIUS") == 0)
								{
									sscanf(cRaedText, "%s %s %f",
										&cDie, &cDie,
										&pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_ColumnInfo->fRadius
									);
								}
								// 縦幅情報読み込み
								else if (strcmp(cHeadText, "VERTICAL") == 0)
								{
									sscanf(cRaedText, "%s %s %f",
										&cDie, &cDie,
										&pmodelAll->v_AttackCollision.at(nCntAttackCollision).p_uni_ColumnInfo->fVertical
									);
								}
							}
						}
...
)
{
	...
	if (pmodelAll != NULL)
	{
		...
		{
			delete pmodelAll->pCharacterCollision;
			pmodelAll->pCharacterCollision = NULL;
		}
		// 攻撃の当たり判定情報の開放
