...
// 時間制限の更新処理
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void C2DPresents::Update_TimeOver(void)
{
	if (!m_pTimeOver) return;
	if (m_pTimeOver->nCntTime >= m_pTimeOver->nMaxTime)
	{
		Release();
	}
	m_pTimeOver->nCntTime++;
}
...
// 時間制限の設定
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void C2DPresents::SetTimeOver(int const & nMaxTime)
{
	if (m_pTimeOver) return;
	m_pTimeOver = new TIMEOVER;
	m_pTimeOver->nMaxTime = nMaxTime;
}
